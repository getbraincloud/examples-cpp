...
void lobby_update()
{
    // Lobby window: auto-sized, centered via pivot
    {
        ...
        {
            // Color the server region label based on how well it matches our ping data.
            // Green  : lobby region ping is within THRESHOLD of our best measured ping (good placement).
            // Red    : lobby region ping is significantly worse than best (likely wrong region).
            // Grey   : no ping data available to compare.
            static const int REGION_MATCH_THRESHOLD_MS = 30;
            ImVec4 regionColor = ImVec4(0.5f, 0.5f, 0.5f, 1.0f); // grey default
            if (!state.pingData.empty())
            {
                auto lobbyIt = state.pingData.find(state.lobby.regionId);
                if (lobbyIt != state.pingData.end())
                {
                    int lobbyPing = lobbyIt->second;
                    int bestPing = INT_MAX;
                    for (const auto &kv : state.pingData)
                        if (kv.second < bestPing)
                            bestPing = kv.second;
                    regionColor = (lobbyPing - bestPing <= REGION_MATCH_THRESHOLD_MS)
                                      ? ImVec4(0.2f, 0.9f, 0.2f, 1.0f)  // green: good region
